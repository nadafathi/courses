(function($){
	...
	function setactive(userid,flag,tname){
		prevclass= 'ajaxHandle sprite16 sprite16-success';
		editclass= 'ajaxHandle sprite16 sprite16-error_delete';
		title = 'Active';
	if(flag){
		prevclass='ajaxHandle sprite16 sprite16-error_delete';
		editclass = 'ajaxHandle sprite16 sprite16-success';
		title = 'Deactive';
	}

	
		//alert(this['flag'+userid]);	
		$.ajax({
			url: 'includes/functions.php',
			type: 'post',
            data: {active:1,userID: userid,flag:flag,tname:tname},
            success: function(data) {
			//console.log(data);
                if (data == 1) {
					$('#usera_'+userid).removeClass(prevclass).addClass(editclass);
					$('#usera_'+userid).attr("alt", title);
					$('#usera_'+userid).attr("title", title);
					ActiveflagArr[userid] = !flag;
					if($("#ch_"+userid).length > 0){
						if(!flag)
							$("#ch_"+userid).attr("disabled", true);
						else
							$("#ch_"+userid).removeAttr("disabled");
					}
	...
	function setpayactive(userid,flag,tname){
		prevclass= 'ajaxHandle sprite16 sprite16-success';
		editclass= 'ajaxHandle sprite16 sprite16-error_delete';
		title = 'Pay';
	if(flag){
		...
		$.ajax({
			url: 'includes/functions.php',
			type: 'post',
            data: {pay:1,userID: userid,flag:flag,tname:tname},
            success: function(data) {
			//console.log(data);
                if (data == 1) {
					$('#userp_'+userid).removeClass(prevclass).addClass(editclass);
					$('#userp_'+userid).attr("alt", title);
					$('#userp_'+userid).attr("title", title);
					PayflagArr[userid] = !flag;
                }else{
					//alert("Error activation");
				} 
            }
