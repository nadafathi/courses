...
function addEditUser_course($Userarr){
    if ($Userarr['ID'] > 0){
        ...
        $sqlstr = "insert into users_courses (arabicname,engname,nationalid,mobilenum,tasneefnum,email,workname,location,studentlevel,participatetype,courseID,hashkey,regdate,isgroup,ispay) VALUE "
                ...
                . "'{$Userarr['studentlevel']}','{$Userarr['participatetype']}',{$Userarr['courseID']},'{$Userarr['hashkey']}','{$Userarr['regdate']}',{$Userarr['isgroup']},0)";
...
if(isset($_POST["submit_frmregsigle"])){
    ...
    $Dataarr['participatetype'] = isset($_POST['participatetype']) ?  $_POST['participatetype'] : "" ;
    
    $Dataarr['isgroup'] =isset($_POST['isgroup']) ?  $_POST['isgroup'] : 0;
 ...
 //var_dump($Dataarr['courseID']);
   // echo addEditUser_course($Dataarr) ;
    if($Dataarr['isgroup'] > 0) {
	$Dataarr['courseID'] = $_POST['coursename'];
    ...
    if($CurrentUserID > 0 &&   $Dataarr['ID'] == 0 ){
         if(sendvarificationEmail($Dataarr,$CurrentUserID,$_POST['coursename']))
            echo $CurrentUserID;
         else
            echo 2;
    }else {
...
function sendvarificationEmail($Userarr,$current_id,$coursesIDs){
	...
	foreach ($coursesIDs as $courseID){
		...
		$coursename .= $idcount < count($coursesIDs) ? "," : "";
		$idcount+=1;
    }
	...
	$email_content = "<html lang='ar'>
				...
				<style>
					...
					body {
					/*background: url('http://doctortproject.org/attendance/images/bg2.jpg') no-repeat;*/
					}
                ...
                <body style='text-align:right; direction:rtl;'>
	               ...
	               <tr>
						...
						<tr>
							<td colspan='2'>في حال تم تسجيلك في الكورسات اون لاين سيتم إرسال رابط القاعه علي الايميل المسجل و في حال واجهة اي مشكله يمكنك التواصل على الرقم التالي: 0560465430</td>
                        </tr>
						
						<tr>
							<td colspan='2'><a href='http://doctortproject.org'>اضغط هنا لمعرفة تفاصيل الأسعار و أرقام الحسابات </a></td>
                        </tr>
